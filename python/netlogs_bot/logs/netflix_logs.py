...
def find_accounts_netflix():
    ...
    #print(list1)
    for i in list1:
        if str(i.find('netflix'))!='-1' or str(i.find('com.netflix.mediaclient'))!='-1':
            valid_list.append(i)

    for x in valid_list:
        line_num = int(x.find('netflix'))
        line_num2 = int(x[line_num:].find('Application'))
        temp1 = x[line_num:line_num2+line_num]
        temp2 = temp1.find('Username')
        done = temp1[temp2:]
        if done !='':
            accounts_netflix.append('{'+done+'}')

    for c in valid_list:
        line_num_android = int(c.find('com.netflix.mediaclient'))
        ...
        if done_android !='':
            accounts_netflix.append('{'+done_android+'}')
